...
class mainApp:
    def __init__(self, Master):
        ...
        self.sendbutton = Button(self.frame2, text="OK", width = 6, height = 1, command=self.sendmessage)
        self.sendbutton.pack(side=LEFT)
        self.sendbutton = Button(self.frame2, text="끝말잇기", width=6, height=1, command=self.game1)
        self.sendbutton.pack(side=LEFT)
        self.sendbutton = Button(self.frame2, text="양자택일", width=6, height=1, command=self.game2)
        self.sendbutton.pack(side=LEFT)
        self.sendbutton = Button(self.frame2, text="GG", width=6, height=1, command=self.gameend)
        self.sendbutton.pack(side=LEFT)

        self.frame3 = Frame(self.mainFrame, padx=2, pady=1)
        self.frame3.pack(fill=X)
        self.textentry = Text(self.frame3, width = 42, height = 4)
        self.textentry.pack(side=LEFT)
        self.sendbutton = Button(self.frame3, text = "Send", width = 6, height =3, command=self.sendmessage)
        self.sendbutton.pack(side=LEFT)
        self.sendbutton = Button(self.frame3, text="Exit", width=6, height=3, command=self.exitprogram)
        self.sendbutton.pack(side=LEFT)
        self.textentry.pack(side = LEFT)
